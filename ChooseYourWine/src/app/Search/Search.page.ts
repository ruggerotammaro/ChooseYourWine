...
export class SearchPage {
  ...
  constructor(private http: Http, private router: Router,private route:ActivatedRoute) {
    ...
    this.tipologia=[];
    this.cantina=[]; 
    this.http.get('http://localhost:8890/sparql?default-graph-uri=http%3A%2F%2Flocalhost%3A8890%2FwineMR&query=PREFIX+vino%3A%3Chttp%3A%2F%2Fw3id.org%2Ffood%2Fontology%2Fdisciplinare-vino%23%3E%0D%0Aselect+DISTINCT+%3Ftipologia%0D%0Awhere%7B%0D%0A%3Fx+vino%3AhaDenominazione+%3Fy.%0D%0A%3Fy+vino%3AhaTipologia+%3Ftipologia.%0D%0A%7D&format=application%2Fsparql-results%2Bjson&timeout=0&debug=on').pipe(map(
  ...
  this.http.get('http://localhost:8890/sparql?default-graph-uri=http%3A%2F%2Flocalhost%3A8890%2FwineMR&query=PREFIX+vino%3A%3Chttp%3A%2F%2Fw3id.org%2Ffood%2Fontology%2Fdisciplinare-vino%23%3E%0D%0Aselect+DISTINCT+%3Fluogo%0D%0Awhere%7B%0D%0A%3Fx+vino%3AhaDenominazione+%3Fy.%0D%0A%3Fy+vino%3A%C3%A9Prodotto+%3Fluogo.%0D%0A%7D&format=application%2Fsparql-results%2Bjson&timeout=0&debug=on').pipe(map(
    res => res.json())).subscribe(data => {this.jsonObject=data;
    this.sparkqlData=JSON.stringify(this.jsonObject);
