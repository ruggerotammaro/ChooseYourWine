...
export class PairingsPage {
  ...
  query(query){
    this.http.get('http://localhost:8890/sparql?default-graph-uri=http%3A%2F%2Flocalhost%3A8890%2FvinoMR&query=PREFIX+vino%3A+%3Chttp%3A%2F%2Fw3id.org%2Ffood%2Fontology%2Fdisciplinare-vino%23%3E+%0D%0A'+query+'%7D&format=application%2Fsparql-results%2Bjson&timeout=0&debug=on').pipe(map(
